...
void loop1() {
    ...
    const uint32_t rc = async_context_execute_sync(&asyncCtx.core, doSomeWork, &value);
    timestamp_exit = to_us_since_boot(get_absolute_time());

    // Quick check of list state right after return
    if (const auto worker = static_cast<async_when_pending_worker*>(asyncCtx.core.when_pending_list)) {
        worker->user_data = reinterpret_cast<void*>(timestamp_exit);  // Store return timestamp as canary
    ...
    if (const auto worker1 = static_cast<async_when_pending_worker*>(asyncCtx.core.when_pending_list)) {
        Serial1.printf("[NO-DELAY] worker:%p next:%p do_work:%p user_data: %llu\n",
            worker1, worker1->next, worker1->do_work, reinterpret_cast<uint64_t>(worker1->user_data));
    }
    delay(7);
}
