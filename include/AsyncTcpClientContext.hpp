...
class AsyncTcpClientContext {
  public:
    AsyncTcpClientContext(tcp_pcb *pcb, const discard_cb_t discard_cb,
        ...
        : _pcb(pcb), _rx_buf(nullptr), _rx_buf_offset(0),
          ...
          _next(nullptr), _sync(AsyncTcpClient::getDefaultSync()) {
...
#endif
        err_t err =
            tcp_connect(_pcb, addr, port, &AsyncTcpClientContext::_s_connected);
        if (err != ERR_OK) {
     ...
     *
     * This method retrieves one byte from the internal receive buffer and
     * advances the buffer offset by one. If the receive buffer is empty, the
     * method returns 0.
     *
     * @return The byte read from the receive buffer, or 0 if the buffer is
     * empty.
     *
    ...
    /**
     * @brief Reads data from the receive buffer into the provided destination
     * buffer.
     *
     * This method attempts to read up to `size` bytes from the internal receive
     * buffer into the buffer pointed to by `dst`. The actual number of bytes
     * read may be less than `size` if there is less data available in the
     * receive buffer.
     *
     * @param dst Pointer to the destination buffer where the data will be
     * copied.
     * @param size The maximum number of bytes to read into the destination
     * buffer.
    ...
    bool
    wait_until_acked(int max_wait_ms = ASYNC_TCP_CLIENT_MAX_FLUSH_WAIT_MS) {
    ...
    size_t write(Stream &stream) {
        ...
        while (stream.available()) {
            // Stream only lets you read 1 byte at a time, so buffer in local
            // copy
            size_t i;
  ...
  protected:
     ...
     *
     * @return true Connection is valid (_pcb exists and state is not CLOSED)
     * @return false Connection is invalid
     *
     * This function checks both the existence of PCB and connection state
     * to determine if TCP operations can be performed.
     */
     ...
     *
     * This function determines the optimal chunk size based on TCP send buffer
     * and applies scaling for memory constraint handling.
     */
     ...
     *
     * This function determines if TCP_WRITE_FLAG_MORE should be set (more data coming)
     * and if TCP_WRITE_FLAG_COPY should be set (based on sync mode).
     */
    [[nodiscard]] uint8_t _get_write_flags(const size_t chunk_size,
     ...
     *
     * This function attempts to write data in chunks, handling TCP buffer constraints
     * and memory limitations through scaling mechanism.
     */
    bool _write_some(const char *datasource, const size_t data_len, size_t *written) const {
        ...
        while (*written < data_len) {
            ...
            const auto next_chunk_size = _calculate_chunk_size(remaining, scale);
    ...
    // We may receive a nullptr as arg in the case when an IRQ happens during a
    // shutdown sequence In that case, just ignore the CB
    ...
    static err_t _s_acked(void *arg, struct tcp_pcb *tpcb, uint16_t len) {
        if (arg) {
            return reinterpret_cast<AsyncTcpClientContext *>(arg)->_acked(tpcb,
                                                                          len);
        } else {
            return ERR_OK;
        }
    ...
    static err_t _s_connected(void *arg, struct tcp_pcb *pcb, err_t err) {
        if (arg) {
            return reinterpret_cast<AsyncTcpClientContext *>(arg)->_connected(
                pcb, err);
        } else {
            return ERR_OK;
        }
