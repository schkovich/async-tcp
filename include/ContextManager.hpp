 ...
 *
 * This file declares the ContextManager class, which serves as the foundation for thread-safe
 * operations across different cores in a multi-core system. It provides:
 *   - A thread-safe environment for executing code on a specific core
 *   - Management of worker tasks that can be scheduled for asynchronous execution
 ...
 * particularly important in embedded systems with limited error recovery options.
 *
...
namespace AsyncTcp {
     ...
     * through the async_context system.
     *
     * @note The void* parameter allows passing arbitrary data to the handler, and the
     * uint32_t return value can be used to indicate success, failure, or other status information.
     */
    typedef uint32_t (*HandlerFunction)(void* param);

    /**
     * @class ContextManager
     * @brief Core component that manages execution contexts and ensures thread safety.
     *
     * The ContextManager provides a stable environment for executing code that needs to run
     * on a specific core or needs to be protected from concurrent access. It serves several
     * key purposes:
     *
     * 1. Thread-Safety: Ensures operations happen in the correct execution context
     * 2. Worker Management: Handles scheduling, tracking, and execution of worker tasks
     * 3. Synchronous Execution: Allows code from one core to be safely executed on another
     * 4. Resource Coordination: Provides locking mechanisms for atomic operations
     *
     ...
     * - Letting the destructor clean up resources when the context is no longer needed
     *
     * Thread safety is guaranteed by the underlying async_context implementation.
     */
    class ContextManager {

        async_context_threadsafe_background_t* m_context; /**< Thread-safe background context for asynchronous operations. */
        async_context_t* m_context_core;                  /**< Pointer to the core asynchronous context. */
        bool initiated = false;                           /**< Flag indicating whether the context has been initialized. */

    public:
        /**
         * @brief Constructs a ContextManager with a specific background context.
         *
         * This constructor initializes a ContextManager instance without activating the context.
         * The context must be explicitly initialized later with initDefaultContext() before use.
         * This separation allows the caller to handle initialization failures appropriately.
         *
         * @param context Reference to a background context structure to be managed
         */
        explicit ContextManager(async_context_threadsafe_background_t& context);

        /**
         * @brief Destructor that cleans up the asynchronous context and related resources.
         *
         * Ensures all resources associated with the context are properly released by:
         ...
         *
         * This cleanup is crucial for preventing resource leaks in long-running applications.
         */
        ~ContextManager();

        /**
         * @brief Adds a persistent worker to the context for ongoing task execution.
         *
         ...
         * other threads to access the context.
         */
         ...
         *
         * This method must be called after construction and before using any other
         * methods that interact with the context. It sets up the internal state and
         * prepares the context for worker registration and execution.
         *
         * @param config Reference to a configuration structure for the background context
         * @return true if initialization succeeded or context was already initialized, false otherwise
         */
         ...
         *
         * @note This is the foundation for patterns like SyncBridge that need guaranteed
         ...
         *
         * Useful for debugging and validation in code that requires the lock to be held.
         */
     ...
     * 
     * This unique_ptr ensures proper cleanup of the ContextManager when it goes out of scope.
     */
