...
namespace AsyncTcp {
     ...
     *
     * This structure serves as a polymorphic base for all payload types that can be
     * passed to SyncBridge for thread-safe execution. It defines a common interface
     * for different types of work data.
     *
     * @note Derived payload types must be final to prevent slicing issues during polymorphic use
     */
    struct SyncPayload {
    ...
    /**
     * @class SyncBridge
     * @brief Thread-safety pattern that provides synchronized access to resources across different execution contexts
     *
     * The SyncBridge design pattern solves a common issue in asynchronous/multi-threaded systems:
     * safely accessing resources that may be modified from different execution contexts.
     * It works by channeling all operations through a synchronized execution mechanism.
     *
     * Key features:
    ...
    class SyncBridge {
        ...
        /**
         * @brief Abstract method that defines the resource-specific operation logic
         * 
         * @param payload A unique pointer to a SyncPayload object containing operation data
    ...
    protected:
        ...
        /**
         * @brief Internal method called by the bridging function to perform the execution
         * 
         * This method delegates to the virtual onExecute() method, allowing derived classes
         * to implement specific behavior while maintaining the bridge pattern structure.
         *
         * @param payload A unique pointer to a SyncPayload object containing operation data
         * @return uint32_t Result code from the operation
         */
    ...
    public:
        ...
        /**
         * @brief Thread-safe execution method that runs the operation in the proper context
         * 
         * This is the main entry point for operations on the resource. It:
         * 1. Packages the payload with this bridge instance
         * 2. Submits the operation to be executed synchronously in the proper context
    ...
    /**
     * @struct BridgingArgs
     * @brief Internal structure that packages a SyncBridge with its payload for execution
     * 
     * This structure allows passing both the bridge instance and its payload through
     * the C-style bridging function interface required by the execution context.
     */
    struct BridgingArgs {
        SyncBridge* bridge{};
