...
namespace async_tcp {
     ...
     * context.
     *
     * This constructor initializes the ContextManager without immediately
     * setting up the asynchronous context. The context must be explicitly
     * initialized by calling initDefaultContext(), allowing the consuming code
     * to control the timing and conditions of initialization and handle
     ...
     *
     * This ensures all resources are properly released even if the user code
     * doesn't explicitly clean up the context.
     */
    ContextManager::~ContextManager() {
        if (initiated) {
     ...
     * configuration.
     *
     * This method prepares the context for worker registration and task
     * execution by:
     * 1. Checking if the context is already initialized (to prevent double
     * initialization)
     * 2. Calling async_context_threadsafe_background_init with the provided
     * configuration
     * 3. Setting the initiated flag if initialization succeeds
     *
     * @param config Reference to a configuration structure for the background
     * context
     * @return true if initialization succeeded or context was already
     * initialized, false on failure
     *
     * By separating initialization from construction, this design allows for
     * explicit error handling and resource management, which is crucial in
     * environments where exceptions are not used.
     */
    ...
    /**
     * @brief Adds a PerpetualWorker to the asynchronous context for ongoing
     * task execution.
     *
     * This method registers a Worker object with the context, allowing it to be
     * triggered for execution when needed. It first checks context validity,
     * then attempts to add the worker to the underlying SDK context.
     *
     * @param worker Reference to the PerpetualWorker instance to be added
     * @return true if the worker was successfully added, false if the context
     ...
     *
     * This prevents the worker from being executed even if setWorkPending() is
     * called. The method checks context validity before attempting the removal.
     *
     * @param worker Reference to the PerpetualWorker instance to be removed
     * @return true if the worker was successfully removed, false if the context
    ...
    /**
     * @brief Removes an ephemeral worker from the scheduled queue before it
     * executes.
     *
     * This method can cancel a previously scheduled ephemeral worker if it
     * hasn't executed yet. It checks context validity before attempting the
     * removal.
     *
     ...
     *
     * This method signals the asynchronous context that the worker has work
     * ready to be executed. The context will call the worker's do_work function
     * as soon as the event loop processes the request.
     *
     ...
     *
     * This is the primary method for ensuring thread-safe execution across
     * cores. It guarantees that the handler function will be executed in the
     * context's core, even if called from a different core. The method blocks
     * until execution is complete, providing a synchronous interface for
     * cross-core operations.
     *
     * This method is the foundation for patterns like SyncBridge that need
     * guaranteed execution context for thread safety.
     *
     ...
     *
     * This diagnostic function checks that the lock is held by the current
     * execution context, useful for debugging and validating code that requires
     * the lock to be held.
     */
